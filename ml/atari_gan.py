# This is a rewritten ripped example from the book deep rl hands on
# Trained on screenshots generated through games played by a random agent.
# Tested with py 3.6 and torch 1.3, training on some modern gpu's requiring recent cuda versions might be a headache.
# But again working reproducable examples seem rare in rl, and this is another.
...
# dimensions to which input image will be rescaled
IMAGE_SIZE = 64
...
# - resize image into predefined size
# - move color channel axis to a first plae
...
class Discriminator(nn.Module):
  def __init__(self, input_shape):
    ...
    # image converges into single number
...
def main():
  ...
  envs = [InputWrapper(gym.make(name)) for name in (
    'Breakout-v0', 
    'AirRaid-v0', 
    'Pong-v0'
  )]
  ...
  for batch_v in iterate_batches(envs):
    ...
    dis_loss = objective(
      dis_output_true_v, true_labels_v
    ) + objective(
